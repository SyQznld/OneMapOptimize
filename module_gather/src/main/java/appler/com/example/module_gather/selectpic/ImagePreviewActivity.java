...
public class ImagePreviewActivity extends AppCompatActivity {
    ...
    private MyPhotoViewPager vpImage;
    ...
    private static ArrayList<String> tempSelectImages;
    ...
    public static void openActivity(Activity activity, ArrayList<String> images,
                                    ArrayList<String> selectImages, boolean isSingle,
        ...
        intent.putExtra(ImageConstants.POSITION, position);
        activity.startActivityForResult(intent, ImageConstants.RESULT_CODE);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vpImage.setCurrentItem(intent.getIntExtra(ImageConstants.POSITION, 0));
    }

    private void initView() {
        vpImage = (MyPhotoViewPager) findViewById(R.id.vp_image);
    ...
    private void initViewPager() {
        ...
        adapter.setOnItemClickListener(new ImagePagerAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(int position, String image) {
                if (isShowBar) {
                    hideBar();
                } else {
                    showBar();
    ...
    private void changeSelect(String image) {
        tvSelect.setCompoundDrawables(mSelectImages.contains(image) ? mSelectDrawable : mUnSelectDrawable, null, null, null);
        if (mSelectImages.contains(image)) {
            tvSelect.setText("已选择");
        } else {
            tvSelect.setText("未选择");
        }
    ...
    public void finish() {
        ...
        intent.putExtra(ImageConstants.IS_CONFIRM, isConfirm);
        setResult(ImageConstants.RESULT_CODE, intent);
        super.finish();
    }
}
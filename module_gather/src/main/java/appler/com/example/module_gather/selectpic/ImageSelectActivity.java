...
public class ImageSelectActivity extends AppCompatActivity {
    ...
    private ArrayList<FolderBean> mFolders;
    ...
    public static void openActivity(Activity activity, int requestCode,
        ...
        intent.putExtra(ImageConstants.MAX_SELECT_COUNT, maxSelectCount);
        intent.putExtra(ImageConstants.IS_SINGLE, isSingle);
        intent.putStringArrayListExtra(ImageConstants.SELECTED, selected);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMaxCount = intent.getIntExtra(ImageConstants.MAX_SELECT_COUNT, 0);
        isSingle = intent.getBooleanExtra(ImageConstants.IS_SINGLE, false);
        mSelectedImages = intent.getStringArrayListExtra(ImageConstants.SELECTED);

//        Log.i(TAG, "image 传值显示图片: " + mSelectedImages.size() + mSelectedImages);
    ...
    private void initListener() {
        ...
        btnPreview.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Log.i(TAG, "image 点击预览后所有图片: " + images.size() + images);
    ...
    private void initFolderList() {
        if (mFolders != null && !mFolders.isEmpty()) {
            isInitFolder = true;
            rvFolder.setLayoutManager(new LinearLayoutManager(ImageSelectActivity.this));
            Log.i("本地选择", "image initFolderList: " + mFolders.size() + mFolders);
            FolderAdapter adapter = new FolderAdapter(ImageSelectActivity.this, mFolders);
    ...
    private void changeTime() {
        ...
        if (firstVisibleItem >= 0 && firstVisibleItem < mAdapter.getData().size()) {
            String image = mAdapter.getData().get(firstVisibleItem);
            String time = new SimpleDateFormat("yyyy/MM/dd").format(new Date());//ImageSelectUtils.getImageTime(image.getTime() * 1000);
            tvTime.setText(time);
            showTime();
    ...
    private void confirm() {
        ...
        ArrayList<String> images = new ArrayList<>();
        for (String image : selectImages) {
            images.add(image);
        }

        //点击确定，把选中的图片通过Intent传给上一个Activity。
        Intent intent = new Intent();
        intent.putStringArrayListExtra(ImageSelectUtils.SELECT_RESULT, images);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageConstants.RESULT_CODE) {

            if (data != null && data.getBooleanExtra(ImageConstants.IS_CONFIRM, false)) {
...
//            Toast.makeText(this, "没有图片", Toast.LENGTH_LONG).show();
        ...
        if (hasWriteContactsPermission == PackageManager.PERMISSION_GRANTED) {
            //有权限，加载图片。
            Log.i(TAG, "image checkPermissionAndLoadImages: "+ "有权限");
            loadImageForSDCard();
        } else {
            //没有权限，申请权限。
            Log.i(TAG, "image checkPermissionAndLoadImages: "+ "没有权限");
    ...
    private void loadImageForSDCard() {
        Log.i(TAG, "image loadImageForSDCard 加载图片: " );
        ImageModelUtils.loadImageForSDCard(this, new ImageModelUtils.DataCallback() {
            @Override
            public void onSuccess(ArrayList<FolderBean> folders) {
                mFolders = folders;
                Log.i("本地选择", "loadImageForSDCard: " + mFolders.size() + mFolders);

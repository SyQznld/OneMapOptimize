...
var createLayers = {
    "raster": function (data) {
         var  sdName = data.name;
         if (data.isShow) {
         //java中图层obj传过来的图层SD卡中命名
         var yx;
         //如果包含“，”，说明多个点查文件，取第一个元素名称为图层名称匹配
         if(sdName.search(",") != -1){
            sdName = sdName.split(",")[0];
            yx = new AIMap.TileLayer.ArcGIS(mapdataPath + getMapdataUrl(sdName), {
                            minZoom: 0,
...
function createShpLayer(obj) {
    ...
    console.log(obj.name + "   " + typeof(obj.name));
        if (obj.name.search(",") != -1) {
            var layers = obj.name.split(',');
            for (var i = 0; i < layers.length;i++) {
                $.ajax({
                    url: obj.path + layers[i] + ".txt",
                    dataType: "json",
                    async: false,
...
function mapClick(e, lat, lng) {
    //var obj = layerGroup;//layerGroup[layerGroup.length - 1];
    ...
    if (layerGroup.length > 0 && obj.identify) {
        ...
        var _returnstr = "";
        _returnstr += LocationAndGetAttrs(obj, curPoint);
...
//                        weight: 10
                    ...
                    _returnstr += "}";
                      console.log("=======_returnstr======" + _returnstr);
                    break;
                }
            }
    }
